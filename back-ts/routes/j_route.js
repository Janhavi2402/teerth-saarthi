...
const Jainism = require('../models/j_model'); // Import the Buddhism model
const router = express.Router(); // Create a router instance


// Route to search temples by query parameters (name, state, or description)
router.get('/jainism/search', async (req, res) => {
  try {
    ...
    const temples = await Jainism.find(query);
...
router.get('/jainism/:id', async (req, res) => {
  try {
    const temple = await Jainism.findById(req.params.id);
    if (!temple) {
...
router.get('/jainism', async (req, res) => {
  try {
    const temples = await Jainism.find({ religion: 'Jainism' });
