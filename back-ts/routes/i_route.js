...
const Islam = require('../models/i_model'); // Import the Buddhism model
const router = express.Router(); // Create a router instance


// Route to search temples by query parameters (name, state, or description)
router.get('/islam/search', async (req, res) => {
  try {
    ...
    const temples = await Islam.find(query);
...
router.get('/islam/:id', async (req, res) => {
  try {
    const temple = await Islam.findById(req.params.id);
    if (!temple) {
...
router.get('/islam', async (req, res) => {
  try {
    const temples = await Islam.find({ religion: 'Islam' });
