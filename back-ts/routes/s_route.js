...
const Sikhism = require('../models/s_model'); // Import the Buddhism model
const router = express.Router(); // Create a router instance


// Route to search temples by query parameters (name, state, or description)
router.get('/sikhism/search', async (req, res) => {
  try {
    ...
    const temples = await Sikhism.find(query);
...
router.get('/sikhism/:id', async (req, res) => {
  try {
    const temple = await Sikhism.findById(req.params.id);
    if (!temple) {
...
router.get('/sikhism', async (req, res) => {
  try {
    const temples = await Sikhism.find({ religion: 'Sikhism' });
