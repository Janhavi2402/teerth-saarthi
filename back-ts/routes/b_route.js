...
const Buddhism = require('../models/b_model'); // Import the Buddhism model
const router = express.Router(); // Create a router instance


// Route to search temples by query parameters (name, state, or description)
router.get('/buddhism/search', async (req, res) => {
  try {
    ...
    const temples = await Buddhism.find(query);
...
router.get('/buddhism/:id', async (req, res) => {
  try {
    const temple = await Buddhism.findById(req.params.id);
    if (!temple) {
...
router.get('/buddhism', async (req, res) => {
  try {
    const temples = await Buddhism.find({ religion: 'Buddhism' });
