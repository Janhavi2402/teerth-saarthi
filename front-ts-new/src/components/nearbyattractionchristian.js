...
export default function NearbyAttractionsChristianPage() {
  ...
  useEffect(() => {
    async function fetchNearbyAttractions() {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:5000/api/christianity/${id}`);
        ...
        {attractions.map((place, index) => (
          <div
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.background = 'linear-gradient(to bottom right, #ffffff, #f2f7fc)';
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'scale(1)';
              e.currentTarget.style.boxShadow = styles.card.boxShadow;
              e.currentTarget.style.background = styles.card.backgroundColor;
            }}
          >
            <h2 style={styles.cardTitle}>{place.name}</h2>
