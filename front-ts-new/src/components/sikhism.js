...
import './search.css';
export default function Sikhism() {
  ...
  const fetchTemples = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/sikhism');
          ...
          <ul className="temple-items">
            {filteredTemples.map((temple) => (
              <li key={temple._id} className="flex items-center justify-between bg-white shadow-md p-4 mb-4 rounded-lg">
                ...
                <Link to={`/sikhism/${temple._id}`} className="text-lg font-medium text-blue-700 hover:underline"onClick={() => handleTempleClick(temple._id)}>
                  {temple.name}
                </Link>
                <button
