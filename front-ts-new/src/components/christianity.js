...
export default function Christianity() {
  ...
  const fetchTemples = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/Christianity');
 ...
 // Runs only once on component mount
  ...
  return (
    <div className="container">
      <h1 className="title">Christianity Temples</h1>
          ...
          <ul className="temple-items">
            {filteredTemples.map((temple) => (
              <li key={temple._id} className="flex items-center justify-between bg-white shadow-md p-4 mb-4 rounded-lg">
                ...
                <Link to={`/christianity/${temple._id}`}  onClick={() => handleTempleClick(temple._id)}   className="text-lg font-medium text-blue-700 hover:underline">
                  {temple.name}
                </Link>
                <button
